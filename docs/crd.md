...
spec:
  ...
  # The reference to the secret containing the connection string to the Azure storage container
  # which must be in the same namespace as the stream definition. The secret must contain a key named ARCANE_CONNECTIONSTRING
  connectionStringRef:
  ...
  sourceSettings:
    ...
    # Name of the table in the source system to be used as a source for the stream
    name: table
  ...
  stagingDataSettings:
    ...
    # Optional override the location of the data.
    dataLocation: s3a://bucket/path

  # The backfill behavior can be set to `reload` or `merge`
  # - override: Overrides the data in the target table with the data from the source table.
  # - merge:  Merges the data from the source table with the data in the target table.
  # see details in the `docs/backfill.md` file
  backfillBehavior: merge

  # The date and time to start backfilling data from.
  # The date should be in the following format: "yyyy-MM-dd'T'HH:mm:ss.SSS'Z'"
  backfillStartDate: 2025-03-03T12:00:00Z
  ...
  tableProperties:
    ...
    # The list of partition expressions to be used for partitioning the target table
